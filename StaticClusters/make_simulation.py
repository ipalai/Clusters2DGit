# This code creates LAMMPS input files for non-fluid clusters as described in the following paper:
#
...
def write_in_script(eps_pp, eps_ll, real, folderPattern, filePattern):
    ...
    for id in centreslist:
        for id2 in centreslist:
            if id2 >= id:
                f.write(f"pair_coeff             {id} {id2} %lf %lf %lf wca \n" % (eps_pp, protein_sigma, protein_sigma))  ## repulsive only
    ...
    for iA in ligands_A:
        for iB in ligands_B:
            f.write(f"pair_coeff             {iA} {iB} {eps_ll} {linker_sigma} {ligand_range + linker_sigma} \n")               ## LAT pY132 - PLC nSH2
    ...
    # With these lines, trajectory is dumped at logarithmically spaced time steps
    if logFlag==1:
        f.write("variable        dumpts equal logfreq(1000,5,10)\n")
        f.write("dump_modify     1 every v_dumpts first yes\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Script for TheoryClusters.")
    ...
    parser.add_argument('--dfactor', '-dfactor', dest='dfactor', action='store', type=float, default=dfactor, help='dfactor = position-of-crosslinker / sigma_p. (dfactor = 0.5 corresponds to the surface of the protein)')
    ...
    filePattern = f"2d_Nowca_ep{eps_pp}_sp{protein_sigma}_el{eps_ll}_sl{linker_sigma}_rl{ligand_range}_d{dfactor}_nA{num_A}_qA{q_A}_nB{num_B}_qB{q_B}_T{Temperature}_Tdamp{LangevinDamping}_dA{density_A:.4f}"
